...
import { useAdminApprovalForm } from "../../admin/hooks/formHooks/useAdminApprovalForm";
...
const UserApprovalFormView = ({ formId, submitted_by, onClose }: Props) => {
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down("md"));
  const [imageErrors, setImageErrors] = useState<Record<string, boolean>>({});

  // TODO: Replace with non-admin hook when API is integrated
              ...
              return (
                <Box key={q.questionId} sx={{ mb: 4 }}>
                  ...
                  {[
                    ...
                    <Typography variant="button">{renderAnswer(q)}</Typography>
                  )}
...
export default UserApprovalFormView;
