...
import { useAdminUploadApprovals } from "../hooks/uploadHooks/useAdminUploadsApprovals";
import { useUploadAdminDetails } from "../hooks/uploadHooks/useUploadAdminDetails";
import AdminUploadsApprovalDetails from "./uploadDetails";
import AdminUploadsApprovalsList from "./uploadList";
import ViewAdminUpload from "./ViewAdminUpload";
enableDayjsUTC();
...
const AdminUploads = () => {
  ...
  const { options: statusOptions, loading: statusLoading } = useDropdown(
    DropdownType.FORM_APPROVAL_STATUS,
  );
  ...
  const {
    ...
    refetch,
    filters,
  } = useAdminUploadApprovals();

  const { loading: detailsLoading, refetch: refetchDetails } =
    useUploadAdminDetails(selectedId);
  ...
  const {
    ...
    saveFilter,
    deleteFilter,
  } = useSavedFilters("APPROVALS_UPLOAD");
  ...
  return (
    <Box sx={{ width: "100%" }}>
            ...
            <>
              ...
              {isMobileViewingDetails &&
                (detailsLoading ? (
                  ...
                  <AdminUploadsApprovalDetails
                    approvalId={selectedId}
                    onBack={() => setMobileViewingDetails(false)}
                    onOpenForm={() => {setIsFormModalOpen(true);setFormOpened(true)}}
            ...
            >
              ...
              {viewMode === "form" && (
                <Grid size={{ xs: 12, md: 5 }}>
                  <ViewAdminUpload
                    ...
                    onClose={() => {setViewMode("details");setFormOpened(false)}}
              ...
              <Grid size={{ xs: 12, md: viewMode === "form" ? 4 : 8 }}>
                {detailsLoading ? (
                  ...
                  <AdminUploadsApprovalDetails
                    approvalId={selectedId}
                    onOpenForm={() => {setViewMode("form");setFormOpened(true);}}
                    onActionSuccess={handleApprovalActionSuccess}
                       formOpened={formOpened}
                  />
                )}
      ...
      <Modal open={isFormModalOpen} onClose={() => {setIsFormModalOpen(false);setFormOpened(false);}}>
        <Box
          // sx={{
          //   width: "65%",
          //   maxWidth: 700,
        ...
        >
          <ViewAdminUpload
            submissionId={selectedId}
            onClose={() => {setIsFormModalOpen(false);setFormOpened(false);}}
          />
        </Box>
      </Modal>
