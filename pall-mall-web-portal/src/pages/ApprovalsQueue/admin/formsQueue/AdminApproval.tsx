...
import { useAdminApprovals } from "../hooks/formHooks/useAdminApprovals";
...
import AdminApprovalFormView from "./AdminApprovalFormView";
import AdminApprovalsList from "./AdminApprovalList";
...
const AdminApprovals = () => {
  ...
  const { options: formPriorityOptions, loading: priorityLoading } =
    useDropdown(DropdownType.FORM_PRIORITY);
  ...
  const { loading: detailsLoading, refetch: refetchDetails } =
    useAdminApprovalsDetails(selectedId);
  ...
  const [formPriority, setFormPriority] = useState<any | null>(null);
  ...
  } = useSavedFilters("APPROVALS_FORM");
  ...
  const handleSelectSavedFilter = (filterId: string) => {
    ...
    const matchedPriority =
      formPriorityOptions.find(
        (opt) => String(opt.value) === String(f.formPriority),
      ) ?? null;
    ...
    setFormPriority(matchedPriority);
  ...
  const buildCurrentFilters = () => ({
    search: search || null,
    formPriority: formPriority?.value ?? null,
  ...
  const buildFilterName = () => {
    ...
    if (formPriority) parts.push(formPriority.label);
  ...
  const hasActiveFilters = () =>
    Boolean(search || formPriority || statusFilter || dateRange);
    ...
    <Box sx={{ width: "100%" }}>
      ...
      >
        <Grid container spacing={2}>
          {/* Search */}
          <Grid size={{ xs: 12, sm: 6, md: isCustomRange ? 6 : 4, lg: isCustomRange ? 3 : 4, xl: isCustomRange ? 3 : 4 }}>
            <BaseTextField
              placeholder="Search by form name or patient name"
              value={search}
              onChange={(e) => {
                const value = e.target.value;
                setSearch(value);
          ...
          {/* status */}
          <Grid size={{ xs: 12, sm: isCustomRange ? 6 : 6, md: isCustomRange ? 6 : 4, lg: isCustomRange ? 3 : 2, xl: isCustomRange ? 3 : 2 }}>
            <BaseSelect
              placeholder="Status"
              value={statusFilter}
          ...
          <Grid
            size={{ xs: 12, sm: 6, md: isCustomRange ? 6 : 4, lg: isCustomRange ? 3 : 2, xl: isCustomRange ? 3 : 2 }}
            ...
            >
              ...
              {viewMode === "form" && (
                <Grid size={{ xs: 12, md: 5 }}>
                  <AdminApprovalFormView
                    formId={selectedApproval?.form?.id ?? null}
                    submitted_by={selectedApproval?.id ?? null}
                    onClose={() => {
                      setViewMode("details");
                      setFormOpened(false);
                    }}
                    onResetButtonState={() => setFormOpened(false)}
                  />
                </Grid>
              )}
