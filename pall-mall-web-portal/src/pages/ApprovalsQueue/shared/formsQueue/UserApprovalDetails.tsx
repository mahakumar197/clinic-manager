...
import { useUserApprovalsDetails } from "../hooks/formHooks/useUserAppprovalsDetails";
import { useUserApprovalComments } from "../hooks/formHooks/useUserApprovalsComments";
...
const UserApprovalDetails = ({
  ...
  // TODO: Replace with non-admin hooks when APIs are integrated
  const { details: userApproval, loading } =
    useUserApprovalsDetails(approvalId);
  ...
  const {
    ...
    reject,
    actionLoading,
  } = useUserApprovalComments(approvalId);

  if (loading || !userApproval) return null;

  const { approval, patient } = userApproval;
  ...
  > = {
    ...
    Urgent: { bg: theme.palette.error.light, text: theme.palette.error.main },
  ...
  type Urgency = "Completed" | "Urgent" | "Pending";
  ...
  const priority = userApproval?.form_flag as Urgency | undefined;
  ...
  return (
    <Box sx={{ height: "100%", overflowY: "auto" }}>
      ...
      >
        ...
        {/* CONDITIONAL AREA */}
        {approval?.form?.form_type === "Health Questionnaire" ? (
        ...
        {userStatusConfig && (
          <Box
            sx={{
              borderRadius: 1.5,
              bgcolor: userStatusConfig.bg,
              border: `1px solid ${userStatusConfig.border}`,
              p: 1.5,
              display: "flex",
              gap: 1,
              alignItems: "center",
          ...
          >
            <CommonIcon
              name={userStatusConfig.icon}
              color={userStatusConfig.text}
            />
            <Typography variant="body2" sx={{ color: userStatusConfig.text }}>
              {userStatusConfig.message}
            </Typography>
          </Box>
        )}
      </Paper>
