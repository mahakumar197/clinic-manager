...
import UserApprovalDetails from "../formsQueue/UserApprovalDetails";
import UserApprovalFormView from "../formsQueue/UserApprovalFormView";
import UserApprovalsList from "../formsQueue/UserApprovalsList";
import { useUserApprovalsDetails } from "../hooks/formHooks/useUserAppprovalsDetails";
import { useUserApprovals } from "../hooks/formHooks/useUserApprovals";
enableDayjsUTC();
...
const ApprovalsList = () => {
  ...
  const { loading: detailsLoading, refetch: refetchDetails } =
    useUserApprovalsDetails(selectedId);
  ...
  const {
    ...
    saveFilter,
    deleteFilter,
  } = useSavedFilters("STAFF_APPROVALS_FORM");
  ...
  const handleSelectSavedFilter = (filterId: string) => {
    ...
    const matchedStatus =
      statusOptions.find(
        (opt) => String(opt.value) === String(f.statusFilter ?? f.status),
      ) ?? null;
  ...
  const hasActiveFilters = () =>
    Boolean(search || statusFilter || dateRange);
  ...
  // TODO: Update these cards to use non-admin API data
  const cards = [
    {
      id: 1,
      title: "Total",
    ...
    <Box sx={{ width: "100%" }}>
      ...
      {isEmpty && (
        <EmptyStateLoader
          ...
          subtitle="Check back later for new approvals"
          height={260}
          icon="FileX"
        />
      )}
